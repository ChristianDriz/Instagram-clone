                    ...
                    <div className="w-full  ">
                        {NavIcons.map((item, idx ) => (
                            <Link href={`/`} key={idx} className="flex gap-4 p-3 my-2 hover:bg-[#f2f2f2] rounded-lg">
                                <Image src={item.icon} alt="home" width={24} height={24} className='object-cover'/> 
                                <span className={`capitalize ${item.isActive ? 'font-bold' : ''} hidden xl:block`}>{item.name}</span> 
                            </Link>
                        ))}
                        <Link href={`/`}  className="flex gap-4 p-3 my-2 hover:bg-[#f2f2f2] rounded-lg">
                            <Image src={Profile[0].icon} alt="home" width={24} height={24} className='object-cover'/> 
